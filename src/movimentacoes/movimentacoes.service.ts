...
export class MovimentacoesService {
  ...
  constructor(
    @InjectRepository(MovimentacaoEstoque) private readonly movimentacaoRepository: Repository<MovimentacaoEstoque>,
 ...
 async create(createMovimentacoesDto: CreateMovimentacaoDto) {
      ...
      const lote = await this.loteRepository.findOne({ where: { id: createMovimentacoesDto.loteId } });
    if (!lote) throw new NotFoundException('Lote não encontrado');

    // Valida se a saída não é maior que a quantidade disponível
    if (createMovimentacoesDto.tipo === 'saida' && createMovimentacoesDto.quantidade > Number(lote.quantidade)) {
      throw new BadRequestException('Quantidade de saída maior que estoque disponível');
    }
    ...
    const movimentacao = this.movimentacaoRepository.create({
      ...
      lote.quantidade = (Number(lote.quantidade) + createMovimentacoesDto.quantidade).toString();
    } else {
      lote.quantidade = (Number(lote.quantidade) - createMovimentacoesDto.quantidade).toString();
    }
